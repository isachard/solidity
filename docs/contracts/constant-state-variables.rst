...
For ``constant`` variables, the value even has to be a constant at compile time and it has to be
...
There can only be a single assignment
and it cannot be read during construction time.
The compiler actually modifies the code by writing that value into the runtime
code before returning it. This is important if you are comparing the
